...
1.  **Template Selection (Optional)**: Fetch and display available templates.
2.  **Rubric Upload**: The user provides the assignment requirements (file or text).
3.  **Dynamic Form**: The user answers specific questions generated by the AI to gather necessary context.
4.  **PDF Output**: The user receives a presigned URL to view/download the final PDF document.
...
```
1. (Optional) Fetch templates → User selects template
2. User uploads rubric
3. Poll until status = "collecting_answers"
4. User answers form questions
5. Poll until:
   - status = "collecting_answers" (with NEW questions) → Go back to step 4
...
## Checkpoint: Fetch Available Templates

Before creating a project, you may want to display available templates to the user.
  ...
  "templates": [
    {
      ...
      "description": "Professional APA 7th edition template using versatile-apa package. Supports academic papers, student assignments, and professional documents."
    },
    {
      "id": "ieee",
      "name": "IEEE Conference/Journal",
      "description": "Standard IEEE format using charged-ieee package. Suitable for conference papers, journals, and technical reports."
    },
    {
      "id": "default",
      "name": "Free / No Template",
      "description": "Minimal template that renders the provided Typst body as-is (no title page, no metadata injection). Includes sensible defaults and robust code block handling."
    }
  ]
}
```

> **Note**: This endpoint proxies the Scribe Heavy API `/v1/templates` and returns the raw payload. Use the `id` field when creating a project.
...
| :--- | :--- | :--- |
| `GET` | `/scribe/templates` | List available Scribe templates (proxied from Heavy API) |
