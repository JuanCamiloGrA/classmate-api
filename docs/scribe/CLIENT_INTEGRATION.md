...
> **⚠️ Important**: The client **never sees markdown or LaTeX**. These are used internally between AI agents. The client only interacts with forms (`formQuestions`) and the final PDF URL (`finalPdfUrl`).
...
> **Note**: The response does **not** include `contentMarkdown`, `currentLatex`, or `finalPdfFileId`. These fields are internal-only and excluded from all API responses.
...
interface ScribeProjectResponse {
  ...
  // currentLatex is EXCLUDED - internal use only
