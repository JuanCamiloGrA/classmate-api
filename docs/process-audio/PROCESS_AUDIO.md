...
```bash
curl -X POST "https://api.classmate.studio/classes/class-123/generate-upload-url" \
...
```bash
curl -X POST "https://api.classmate.studio/classes/class-123/process-audio" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "https://api.classmate.studio/classes/class-123" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
